...
fn improved_nnf(
    roi_pair: &[(BoxedRef<'_, Mat>, BoxedRef<'_, Mat>)],
    current_ssd: f32,
) -> Option<f32> {
    roi_pair.iter().fold(None, |_, (proposed, patch)| {
        let new_ssd = sum_squared_differences(&proposed.try_clone().ok()?, &patch.try_clone().ok()?).ok()?;
